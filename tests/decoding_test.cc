...
namespace {
  ...
  struct HasPhraseBiasStartBias : std::false_type {};

  template <typename T>
  struct HasPhraseBiasStartBias<T, std::void_t<decltype(&T::start_bias)>> : std::true_type {};

  template <typename T, typename = void>
  struct HasPhraseBiasMode : std::false_type {};

  template <typename T>
  struct HasPhraseBiasMode<T, std::void_t<decltype(&T::mode)>> : std::true_type {};
}
...
TEST(PhraseBiasTest, PhraseBiasPathDefaults) {
  ...
  EXPECT_FALSE(HasPhraseBiasStartBias<ctranslate2::models::PhraseBiasPath>::value);
  EXPECT_FALSE(HasPhraseBiasMode<ctranslate2::models::PhraseBiasPath>::value);
}
...
TEST(PhraseBiasTest, TrieSharedPrefixReturnsBoth) {
  ...
  EXPECT_EQ(boost({1}), (std::map<size_t,float>{{2, 0.5f}}));        // [A] -> B (sum two paths: 0.25+0.25)
  EXPECT_EQ(boost({1, 2}), (std::map<size_t,float>{{3, 0.25f}, {4, 0.25f}}));  // [A,B] -> C,D
  EXPECT_EQ(boost({8}), (std::map<size_t,float>{{9, 0.5f}}));        // [X] -> Y
  EXPECT_TRUE(boost({7}).empty());                                  // Mismatch.
  EXPECT_TRUE(boost({}).empty());                                   // No suffix.
}
...
TEST(PhraseBiasTest, TrieMinPrefixLenSkipsShortMatch) {
  ...
  EXPECT_TRUE(out1.empty());                        // [A] alone does not boost.
...
TEST(PhraseBiasTest, TrieSkipsTooShortEntry) {
  PhraseBiasTrie trie;
  trie.add(PhraseBiasEntry{{5}, 0.5f, 1});  // 1-token path has no continuation -> skip.
  std::map<size_t, float> out;
...
TEST(PhraseBiasTest, ProcessorBoostsNextTokenOnSuffixMatch) {
  // vocab=5, batch=1. All logits are 0.
...
TEST(PhraseBiasTest, ProcessorOverlapSumsAndClamps) {
  StorageView logits({1, 5}, std::vector<float>(5, 0.f));
  DisableTokens disable(logits, std::numeric_limits<float>::lowest());
  // Both phrases boost [A]->B: 0.6 + 0.6 = 1.2 -> clamp 1.0.
...
TEST(PhraseBiasTest, ProcessorDefaultMaxTokenDeltaClampsAtTwo) {
  StorageView logits({1, 5}, std::vector<float>(5, 0.f));
  DisableTokens disable(logits, std::numeric_limits<float>::lowest());
  std::vector<PhraseBiasEntry> entries = {{{1, 2}, 1.2f, 1}, {{1, 2}, 1.2f, 1}};
  PhraseBiasProcessor proc(entries);
  StorageView seq({1, 1}, std::vector<int32_t>{1});
  proc.apply(1, logits, disable, seq, {0}, nullptr);
  StorageView expected({1, 5}, std::vector<float>{0, 0, 2.0f, 0, 0});
  expect_storage_eq(logits, expected);
}

TEST(PhraseBiasTest, ProcessorDefaultMaxTokenDeltaClampsNegativeAtMinusTwo) {
  StorageView logits({1, 5}, std::vector<float>(5, 0.f));
  DisableTokens disable(logits, std::numeric_limits<float>::lowest());
  std::vector<PhraseBiasEntry> entries = {{{1, 2}, -1.2f, 1}, {{1, 2}, -1.2f, 1}};
  PhraseBiasProcessor proc(entries);
...
TEST(PhraseBiasTest, ConvertEmptyModelOptionToNoEntries) {
  using namespace ctranslate2::models;
  EXPECT_TRUE(to_phrase_bias_entries({}).empty());

  std::vector<PhraseBias> biases(1);
  EXPECT_TRUE(to_phrase_bias_entries(biases).empty());
}
...
// CPU/GPU parity: logits use the target device/dtype, sequences stay on host(int32)
// just like the actual generate contract.
...
TEST_P(PhraseBiasProcessorFPTest, CpuGpuParity) {
  ...
  // batch=2 covers row-wise behavior with batch>1; this is not beam end-to-end.
  // vocab=6, all logits are 0.
  StorageView logits({2, 6}, std::vector<float>(12, 0.f), device);
  logits = logits.to(dtype);
  DisableTokens disable(logits, std::numeric_limits<float>::lowest());

  // Two paths share prefix [1,2,*] -> verify overlap summation.
  ...
  // sequences always stay on host(CPU). row0 suffix [...,1] boosts token2 (0.3+0.3=0.6).
  //                                      row1 suffix [1,2] boosts token3 and token4 (0.3 each).
  ...
  StorageView expected({2, 6}, std::vector<float>{
      0, 0, 0.6f, 0,    0,    0,    // row0: token2 += 0.6 (summed)
